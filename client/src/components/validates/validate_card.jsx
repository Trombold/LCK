...
export default function ValidateCard({
  ...
  const deleteValidate = async () => {
    ...
    if (resConfirm) {
      ...
      setIsVisible(false);
      toast.success("Válida eliminada con exito");
    }
  };
  ...
  return (
    ...
    >
      <div className="px-6 py-4">
        ...
        <p className="text-gray-700 dark:text-gray-300 text-base">{timeInit}</p>
      </div>
      <div className="px-6 pt-4 pb-2 flex flex-wrap justify-start gap-4">
        <Link
          to={`/manage/validates/${id}`}
...
ValidateCard.propTypes = {
  ...
  timeInit: PropTypes.string.isRequired,
};
